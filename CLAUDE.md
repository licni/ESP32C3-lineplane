...
- **開發板基準設定(GG 2026-09-14,優先於上面的備份/還原規則)**:板子的設定一律用 `test_logs/基準設定碼.txt` 那組設定備份碼.
  需要設定或還原板子時,不管板上現有設定是什麼,不用詢問:待機時放棄未儲存變更 → `POST /api/backup/apply`(text=碼)→ `POST /api/save` → `GET /api/backup` 與基準碼比對.
  GG 說要改基準時才換碼. **韌體出廠值也等於這組**(settings.cpp defaultShared/defaultProfile,第 6 組名稱「測試」,出廠飛行風格第 6 組);換基準碼時出廠值一起改.
