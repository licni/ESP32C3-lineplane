...
> 必須加裝 **GPIO21 起飛安全開關**(見[接線](#接線)):起飛程序照常開始,按下安全開關才會開始倒數.
...
| 情況 | 行為 |
|---|---|
| GPIO21 安全開關沒按下(或沒接,斷線) | 起飛程序會開始,但不會倒數,馬達不轉;按下開關(放穩前後都可以)才倒數. 超過 3 分鐘(可調)沒按自動取消 |
| 設定改了還沒儲存 | 拒絕起飛 |
...
| 安全開關 | 微動開關 + 1kΩ 電阻(**必要**) |
| 收腳舵機 | 選用 |
| 供電 | 板上降壓板,支援 6~9V 的電變 BEC;收腳舵機從 BEC 分電,不經過控制器. 可同時接 USB(二極體保護). 電壓不穩會讓板子重開(馬達立刻停) |
...
| GPIO21 | **安全開關** → 1kΩ → GND | 按下(接地)才開始倒數. 開機瞬間這腳會被拉高,串電阻保護腳位 |
| GPIO5 | MPU6050 SDA | 腳位可在 `src/pins_config.h` 修改 |
| GPIO6 | MPU6050 SCL | |
| GPIO3 | 收腳舵機訊號線 | 選用 |
| GPIO8 | 板載 LED | 狀態燈號 |
| 電源輸入(降壓板)/ GND | 電變 BEC 6~9V | 收腳舵機直接從 BEC 分電 |
...
起飛程序(推飛機,網頁開始,上電倒數)照常開始,**按下安全開關才開始倒數**;放穩前後按都可以,按一下就記住,超過安全開關等待上限(出廠 3 分鐘)沒按就自動取消. 燈號:待機長亮,等待放穩/等安全開關慢閃,倒數快閃,飛行中熄滅,正常結束閃 2 下,撞擊斷電閃 3 下.

## 快速開始

> [!TIP]
> 已經拿到燒好韌體的控制器?直接看 **[使用說明書](docs/使用說明書.md)**,從接電變開始一步一步教.
...
**4. 起飛**:推一下飛機(或網頁「開始起飛程序」)→ 放穩 → 按下安全開關 → 倒數 → 起飛.
...
- 網頁「系統 → 韌體更新 → 檢查更新」,網站上的版本比板子新才提示,板子自己下載,比對大小與 SHA-256 檢查碼後安裝.
...
| 文件 | 內容 |
|---|---|
| [使用說明書](docs/使用說明書.md) | **拿到控制器之後從這裡開始**:接線,連上板子,每個參數的用法與設計用意,起飛操作,疑難排解 |
